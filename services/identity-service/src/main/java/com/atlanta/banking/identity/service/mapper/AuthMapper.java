...
public interface AuthMapper {
    ...
    @Mapping(target = "roles", expression = "java(employee.getRoles().stream().map(r -> r.getName()).collect(java.util.stream.Collectors.toSet()))")
    @Mapping(target = "tokenType", constant = "Bearer")
    LoginResponse toLoginResponse(Employee employee, String accessToken, long expiresIn);
